...
PyQt5-based interface for 7" Waveshare touchscreen
"""
...
from PyQt5.QtWidgets import (
    ...
    QGroupBox, QSpinBox, QDoubleSpinBox, QTimeEdit, QCheckBox,
...
# Screen dimensions for Waveshare 7" (800x480)
SCREEN_WIDTH = 800
SCREEN_HEIGHT = 480
...
class TemperatureDisplay(QFrame):
    ...
    def __init__(self, label: str, unit: str = "°F"):
        ...
        self.value_label.setStyleSheet("font-size: 24px; font-weight: bold; color: #74c0fc;")
    ...
    def set_value(self, value: Optional[float], style: str = None):
        ...
        if style:
            self.value_label.setStyleSheet(f"font-size: 24px; {style}")
...
class DashboardTab(QWidget):
    ...
    def _setup_ui(self):
        ...
        snowmelt_frame = QFrame()
        snowmelt_layout = QVBoxLayout(snowmelt_frame)
        self.snowmelt_status = QLabel("Snowmelt: IDLE")
        self.snowmelt_status.setStyleSheet("font-size: 16px; font-weight: bold;")
        snowmelt_layout.addWidget(self.snowmelt_status)
        
        dhw_frame = QFrame()
        dhw_layout = QVBoxLayout(dhw_frame)
        self.dhw_status = QLabel("DHW: IDLE")
        self.dhw_status.setStyleSheet("font-size: 16px; font-weight: bold;")
        dhw_layout.addWidget(self.dhw_status)
        
        eco_frame = QFrame()
        eco_layout = QVBoxLayout(eco_frame)
        self.eco_status = QLabel("Eco: OFF")
        self.eco_status.setStyleSheet("font-size: 16px; font-weight: bold;")
        eco_layout.addWidget(self.eco_status)
        
        status_layout.addWidget(snowmelt_frame)
        status_layout.addWidget(dhw_frame)
        status_layout.addWidget(eco_frame)
        ...
        self.time_label.setStyleSheet("font-size: 18px; font-weight: bold; color: #e94560;")
    ...
    def update_display(self, state: ControlState):
        ...
        color = snowmelt_colors.get(state.snowmelt_state, "#adb5bd")
        self.snowmelt_status.setText(f"Snowmelt: {state.snowmelt_state.value.upper()}")
        self.snowmelt_status.setStyleSheet(f"font-size: 16px; font-weight: bold; color: {color};")
        
        color = snowmelt_colors.get(state.dhw_state, "#adb5bd")
        self.dhw_status.setText(f"DHW: {state.dhw_state.value.upper()}")
        self.dhw_status.setStyleSheet(f"font-size: 16px; font-weight: bold; color: {color};")
        
        eco_color = "#51cf66" if state.eco_active else "#adb5bd"
        self.eco_status.setText(f"Eco: {'ACTIVE' if state.eco_active else 'OFF'}")
        self.eco_status.setStyleSheet(f"font-size: 16px; font-weight: bold; color: {eco_color};")
...
class EquipmentTab(QWidget):
    ...
    def _setup_ui(self):
        ...
        glycol_pump = EquipmentControl("glycol_pump", "Glycol Pump", "Circulates glycol through slab")
        primary_pump = EquipmentControl("primary_pump", "Primary Pump", "Circulates water through heat exchanger")
        bypass_valve = EquipmentControl("bypass_valve", "Bypass Valve", "Controls glycol flow to heat exchanger")
        ...
        dhw_pump = EquipmentControl("dhw_pump", "DHW Recirculation Pump", "Circulates domestic hot water")
...
class SetpointsTab(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.snowmelt_enabled = QCheckBox("Enable Snowmelt")
        ...
        snowmelt_layout.addWidget(QLabel("High Setpoint:"), 1, 0)
        ...
        snowmelt_layout.addWidget(QLabel("Delta T:"), 2, 0)
        ...
        self.glycol_low_label = QLabel("Low Setpoint: --")
        self.glycol_low_label.setStyleSheet("color: #adb5bd;")
        ...
        self.dhw_enabled = QCheckBox("Enable DHW")
        ...
        dhw_layout.addWidget(QLabel("High Setpoint:"), 1, 0)
        ...
        dhw_layout.addWidget(QLabel("Delta T:"), 2, 0)
        ...
        self.dhw_low_label = QLabel("Low Setpoint: --")
        self.dhw_low_label.setStyleSheet("color: #adb5bd;")
        ...
        self.eco_enabled = QCheckBox("Enable Eco Mode")
        ...
        eco_layout.addWidget(QLabel("High Setpoint:"), 1, 0)
        ...
        eco_layout.addWidget(QLabel("Delta T:"), 2, 0)
        ...
        eco_layout.addWidget(QLabel("Start Time:"), 3, 0)
        ...
        eco_layout.addWidget(QLabel("End Time:"), 4, 0)
        ...
        content_layout.addWidget(snowmelt_group)
        content_layout.addWidget(dhw_group)
    ...
    def _on_glycol_changed(self):
        ...
        low = high - delta
        self.glycol_low_label.setText(f"Low Setpoint: {low:.1f} °F")
    ...
    def _on_dhw_changed(self):
        ...
        low = high - delta
        self.dhw_low_label.setText(f"Low Setpoint: {low:.1f} °F")
    ...
    def update_display(self, state: ControlState):
        ...
        for widget in [self.snowmelt_enabled, self.dhw_enabled, self.eco_enabled,
        ...
        self.glycol_low_label.setText(f"Low Setpoint: {state.glycol_setpoints.low_temp:.1f} °F")
        
        self.dhw_high.setValue(state.dhw_setpoints.high_temp)
        self.dhw_delta.setValue(state.dhw_setpoints.delta_t)
        self.dhw_low_label.setText(f"Low Setpoint: {state.dhw_setpoints.low_temp:.1f} °F")
